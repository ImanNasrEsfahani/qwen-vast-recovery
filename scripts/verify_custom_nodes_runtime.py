...
def main():
    ...
    for item in data['items']:
        ...
        for asset in item.get('required_assets',[]):
            p=comfy/asset['path']
            ok=p.exists() and p.stat().st_size>=int(asset.get('min_size',1))
            if ok and asset.get('sha256'):
                ok=sha256_file(p).lower()==asset['sha256'].lower()
            print(('✓' if ok else '✗'),p)
        ...
        if item['id']=='reactor':
            try:
                ...
                if selection.get('hardware',{}).get('cuda_available') and 'CUDAExecutionProvider' not in providers:
                ...
                expected=[
                    ...
                    'source_faces_index','console_log_level'
                ]
